...
function Player(x, y, direction) {
	...
	this.inventory = [
		new Bitmap('assets/righthand.png', 320, 332),
		new Bitmap('assets/righthand.png', 319, 320)
	];
...
function Map(size) {
	...
	this.skybox = new Bitmap('assets/skybox03.jpg', 4000, 1290);
	this.wallTexture = new Bitmap('assets/wall_texture04.jpg', 1024, 1024);
	this.floorTexture = new Bitmap('assets/floor_texture.jpg', 391,392);
	this.light = 0;
	this.objects = [];
}
...
Map.prototype.randomize = function() {
	...
	flagX=Math.random() * (45) + 5;
	flagY=flagX+0.5;
	this.wallGrid[Math.floor(flagX)]=0;
};
...
map.addObject({
	color: 'brown',
	texture: new Bitmap('assets/ghost.png', 639, 1500),
	height: .7,
	width: .225,
	floorOffset: 0,
	speed: .1,
...
map.addObject({
	color: 'green',
	texture: new Bitmap('assets/flag.png', 639, 1500),
	height: .7,
	width: .225,
	floorOffset: 0,
	speed: .1,
...
loop.start(function frame(seconds) {
	...
	//console.log("flagx:"+flagX+"- x:"+player.x+";y:"+player.y);
	if(Math.floor(player.x)==Math.floor(flagX) && Math.floor(player.y)==Math.floor(flagY)){
		console.log("flag trouvé!!!")
...
function drawGameOver(canvas){
	...
	var imgGameOver = new Image();   // Crée un nouvel objet Image
	imgGameOver.src = 'assets/youwin.jpg';
	canvasContext.drawImage(imgGameOver,100,100,window.innerWidth-200,window.innerHeight-200)
